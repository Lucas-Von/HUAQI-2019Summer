...
def invest_algorithm():
    ...
    for key in his_val_data_df:
        ...
        if judge_valpercent_range(valpercent) in ['最极度低估', '极度低估'] or (
                judge_valpercent_range(valpercent) in ['低估', '正常低估'] and val0 < his_val_lst0[-1]):
...
def buy_sell_result(buy_lst: list, sell_lst: list, money_expected_deploy: float, total_cost: float, total_number: int,
                    qdiifund_position: list):
    """给出最终的购买/卖出结果输出"""
...
def main(jsondata):
    ...
    try:
        ...
        print(buy_sell_result(buy_lst, sell_lst, c['target'], c['sum'], c['num'], c['hold']))
        return 0
    except:
        return -1
